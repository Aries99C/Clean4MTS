# -*- coding:utf-8 -*-
# @Author : Genglong Li
# @Time : 2023/3/29 21:55

import DQDiscovery.LearnCRR.database as database
import copy
import numpy as np
from DQDiscovery.LearnCRR import lineartree3
...
def partition(rangex: Range, part_meth, attr, adaptive_val=None):
    if part_meth == 'binary':
        return rangex._binary(attr)
    elif part_meth == 'triary':
        return rangex._triary(attr)
    elif part_meth == 'median':
        return [rangex._clone().limit_dom(attr, rangex.l[attr], adaptive_val),
                rangex._clone().limit_dom(attr, adaptive_val, rangex.r[attr])]
    else:
        return [None]
...
def dataset(db: database.Database, src: list, target: list, rate=0.1, const_factor=True, illegal=-100, samp=1.0):
    ...
    train_id, test_id = np.sort(ids[:int(np.shape(X)[0] * (1 - rate) * samp)]), np.sort(
        ids[int(np.shape(X)[0] * (1 - rate) * samp):int(np.shape(X)[0] * samp)])
...
def test_model(models, trainX, trainY, rangex: Range, eps: float, mod):
    ...
    for model in models:
        if mod == 'linear':
            bias = np.array(trainY - model.predict(trainX))
        elif mod == 'bayesian':
...
# model sharing NG
def alg1_discover_ns(models, db, target, src, source_range: Range, partition_attr, mod='linear', part_meth='binary',
                     eps=0.1, edge_sz=30):
...
def alg1_discover(models, db, target, src, source_range: Range, partition_attr, mod='linear', part_meth='binary',
                  eps=0.1, edge_sz=30, source_ranges=None):
    trainX, trainY, testX, testY, train_id, test_id = dataset(db, src, target)
    print(np.shape(trainX), np.shape(trainY), np.shape(testX), np.shape(testY))
    if source_ranges:
        que = source_ranges
    ...
    while que:
        ...
        if error <= eps:
            ...
            if np.shape(trX)[0] <= edge_sz or loss <= eps:
                ...
                if part_meth == 'median':
                    adp = np.sort(subX[:, partition_attr])
                else:
                    adp = 0
...
from DQDiscovery.LearnCRR.lineartree2 import LinearTreeRegressor
...
def alg1_discover_rel(db, src, target, depth=10):
    ...
    mask = mask > 0
    if mask.sum() > 0:
        rmse = np.sqrt(mean_squared_error(pred[mask], testY[mask]))
    else:
        rmse = 0.
        ...
        while candidate:
            ...
            for i, rule in zip(range(len(crr)), crr):
                ...
                for attr in rule[-1].l:
                    if t[mp[attr]] >= rule[-1].l[attr] and t[mp[attr]] <= rule[-1].r[attr]:
                        continue
                    else:
                        flag = True
                        break
...
def air(sz=1000):
    schema = [('Time', 'Value'), ('CO(GT)', 'Value'), ('PT08.S1(CO)', 'Value'), ('NMHC(GT)', 'Value'),
              ('C6H6(GT)', 'Value'), ('PT08.S2(NMHC)', 'Value'), ('NOx(GT)', 'Value'), ('PT08.S3(NOx)', 'Value'),
              ('NO2(GT)', 'Value'), ('PT08.S4(NO2)', 'Value'), ('PT08.S5(O3)', 'Value'), ('T', 'Value'),
              ('RH', 'Value'), ('AH', 'Value')]
    ...
    db.add_table("…/…/…/datasets/AirQualityUCI_t3.csv",
                 ...
                 ";", False, data_size)
    db.value_type(
        [('Time', 'Float'), ('CO(GT)', 'Float'), ('PT08.S1(CO)', 'Float'), ('NMHC(GT)', 'Float'), ('C6H6(GT)', 'Float'),
         ('PT08.S2(NMHC)', 'Float'), ('NOx(GT)', 'Float'), ('PT08.S3(NOx)', 'Float'), ('NO2(GT)', 'Float'),
         ('PT08.S4(NO2)', 'Float'), ('PT08.S5(O3)', 'Float'), ('T', 'Float'), ('RH', 'Float'), ('AH', 'Float')]
    )
    r = Range(None, None)
    r.build(db, [0])
    print(r.l, r.r)
    mod = 'linear'
    crr, testX, testY, model, test_id = alg1_discover(None, db, [11], [0], r, partition_attr=0, part_meth='binary',
                                                      mod=mod, eps=1.0, edge_sz=5)
    print(len(crr))
...
def air_crr(sz=1000):
    schema = [('Time', 'Value'), ('CO(GT)', 'Value'), ('PT08.S1(CO)', 'Value'), ('NMHC(GT)', 'Value'),
              ('C6H6(GT)', 'Value'), ('PT08.S2(NMHC)', 'Value'), ('NOx(GT)', 'Value'), ('PT08.S3(NOx)', 'Value'),
              ('NO2(GT)', 'Value'), ('PT08.S4(NO2)', 'Value'), ('PT08.S5(O3)', 'Value'), ('T', 'Value'),
              ('RH', 'Value'), ('AH', 'Value'), ('pre1', 'Value'), ('pre2', 'Value'), ('pret1', 'Value'),
              ('pret2', 'Value')]
    db = database.Database()
    data_size = sz
    ...
    db.value_type(
        [('Time', 'Float'), ('CO(GT)', 'Float'), ('PT08.S1(CO)', 'Float'), ('NMHC(GT)', 'Float'), ('C6H6(GT)', 'Float'),
         ('PT08.S2(NMHC)', 'Float'), ('NOx(GT)', 'Float'), ('PT08.S3(NOx)', 'Float'), ('NO2(GT)', 'Float'),
         ('PT08.S4(NO2)', 'Float'), ('PT08.S5(O3)', 'Float'), ('T', 'Float'), ('RH', 'Float'), ('AH', 'Float'),
         ('pre1', 'Float'), ('pre2', 'Float'), ('pret1', 'Float'), ('pret2', 'Float')]
    )
    r = Range(None, None)
    r.build(db, [0])
    # print(r.l, r.r)
    # mod = 'bayesian'
    mod = 'linear'
    crr, testX, testY, model, test_id = alg1_discover(None, db, [1], [0, -1, -2, -3, -4], r, partition_attr=0,
                                                      part_meth='binary', mod=mod, eps=0.7, edge_sz=5)
...
def bird_crr(sz=1000, depth=10):
    ...
    return (np.array(alg1_discover_rel(db, src=[2, 3], target=[0], depth=depth)) + np.array(
        alg1_discover_rel(db, src=[2, 3], target=[1], depth=depth))) / 2.0
...
def bird_crr_sc(sz=1000, depth=10):
    ...
    mod = 'linear'
    # return np.array(alg1_discover_rel(db, src=[2,3], target=[1]))
    return (np.array(alg1_discover_rel(db, src=[2, 3, -1, -2, -3, -4], target=[0], depth=depth)) + np.array(
        alg1_discover_rel(db, src=[2, 3, -1, -2, -3, -4], target=[1], depth=depth))) / 2.0
...
def bird_crr_sc2(sz=1000):
    ...
    for i in db.dom[2]:
        ...
        crr, testX, testY, model, test_id = alg1_discover(None, db, [0], [3, -1, -2, -3, -4], r, partition_attr=3,
                                                          part_meth='binary', mod=mod, eps=0.7, edge_sz=5)
        pac = alg2_compation(db, crr, model, source=3)
        tmp = np.array(test_com(db, testX, testY, crr, mod, test_id=test_id, maps=pac))[0]
        crr, testX, testY, model, test_id = alg1_discover(None, db, [1], [3, -1, -2, -3, -4], r, partition_attr=3,
                                                          part_meth='binary', mod=mod, eps=0.7, edge_sz=5)
        pac = alg2_compation(db, crr, model, source=3)
        tmp += np.array(test_com(db, testX, testY, crr, mod, test_id=test_id, maps=pac))[0]
        if ans is None:
...
def bird_regressiontree(sz=1000, depth=5):
    ...
    for i in db.dom[2]:
        ans.append(r._clone().limit_dom(2, i, i))
    mod = 'linear'
    return (np.array(regressionTree(db, src=[2, 3], target=[0], depth=depth)) + np.array(
        regressionTree(db, src=[2, 3], target=[1], depth=depth))) / 2.0
...
def bird_recur(sz=1000):
    ...
    for i in db.dom[2]:
        dbi = db.filter(condition=[(2, i, '=', 3)])
        if ans is None:
            ans = (np.array(deltaTRecur(db, target=0, temporal=3)) + np.array(
                deltaTRecur(db, target=1, temporal=3))) / 2.0
        else:
            ans += (np.array(deltaTRecur(db, target=0, temporal=3)) + np.array(
                deltaTRecur(db, target=1, temporal=3))) / 2.0
    # crr = pack_crr(db, crr, model, source=0)
...
def bird_AR(sz=1000):
    ...
    for i in db.dom[2]:
        dbi = db.filter(condition=[(2, i, '=', 3)])
        if ans is None:
            ans = (np.array(AR(dbi, 3, 0)) + np.array(AR(dbi, 3, 1))) / 2.0
        else:
            ans += (np.array(AR(dbi, 3, 0)) + np.array(AR(dbi, 3, 1))) / 2.0
    # print(len(crr))
    # print(len(alg2_compation(crr))
...
def abalone_crr(sz=1000, depth=5):
    db = database.Database()
    schema = [("sex", "Enumerate"), ("length", "Value"), ("diameter", "Value"), ("height", "Value"),
              ("whole_weight", "Value"),
    ...
    db.value_type([("length", "Float"), ("diameter", "Float"), ("height", "Float"), ("whole_weight", "Float"),
                   ("shucked_weight", "Float"), ("viscera_weight", "Float"), ("shell_weight", "Float"),
                   ("rings", "Float")])
    mod = 'linear'
...
def power_crr(sz=1000):
    schema = [('Time', 'Value'), ('Global_active_power', 'Value'), ('Global_reactive_power', 'Value'),
              ('Voltage', 'Value'), ('Global_intensity', 'Value'), ('Sub_metering_1', 'Value'),
              ('Sub_metering_2', 'Value'), ('Sub_metering_3', 'Value'), ('pre1', 'Value'), ('pre2', 'Value'),
              ('pret1', 'Value'), ('pret2', 'Value')]
    db = database.Database()
    data_size = sz
    db.add_table("…/…/…/datasets/power_t3.csv", schema, ";", first_line_omit=False, max_index=sz)
    db.value_type(
        [('Time', 'Float'), ('Global_active_power', 'Float'), ('Global_reactive_power', 'Float'), ('Voltage', 'Float'),
         ('Global_intensity', 'Float'), ('Sub_metering_1', 'Float'), ('Sub_metering_2', 'Float'),
         ('Sub_metering_3', 'Float'), ('pre1', 'Float'), ('pre2', 'Float'), ('pret1', 'Float'), ('pret2', 'Float')]
    )
    r = Range(None, None)
    r.build(db, [0])
    # print(r.l, r.r)
...
def tax_crr(sz=1000):
    ...
    schema = [('FName', 'Enumerate'), ('LName', 'Enumerate'), ('Gender', 'Enumerate'), ('AreaCode', 'Enumerate'),
              ('Phone', 'Enumerate'), ('City', 'Enumerate'), ('State', 'Enumerate'), ('Zip', 'Enumerate'),
              ('MaritalStatus', 'Enumerate'),
...
def birdmap():
    ...
    typex = [('decimalLatitude', 'Float'), ('decimalLongitude', 'Float'), ('day', 'Int'), ('month', 'Int'),
             ('year', 'Int'), ('date', 'Int')]
    db.value_type(typex)
