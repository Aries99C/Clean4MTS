...
from DQDiscovery.LearnCRR.lineartree2._criterion import SCORING
from DQDiscovery.LearnCRR.lineartree2._criterion import mse, rmse, mae, poisson
from DQDiscovery.LearnCRR.lineartree2._criterion import hamming, crossentropy
...
def _parallel_binning_fit(split_feat, _self, X, y,
    ...
    split_t = None
    split_col = None
    left_node = (None, None, None, None, 0., -1)
    right_node = (None, None, None, None, 0., -1)
        ...
        for q in _bins:
            ...
            if total_loss < loss:
                ...
                left_node = (model_left, loss_left, wloss_left,
                             n_left, largs_left['classes'], 0., -1)
                right_node = (model_right, loss_right, wloss_right,
                              n_right, largs_right['classes'], 0., -1)
...
def _ext_model_fitting(X, y, model_id, col, q, _self):
    feval = CRITERIA[_self.criterion]
    mask = (X[:, col] > q)
    left_mesh = np.ix_(~mask, _self._linear_features)
    right_mesh = np.ix_(mask, _self._linear_features)
    model = _self._ext_models[model_id]
    lp = model.predict(X[left_mesh])
    rp = model.predict(X[right_mesh])
    l_bias = y[~mask] - lp
    r_bias = y[mask] - rp
    lm = (np.max(l_bias) + np.min(l_bias)) / 2.0
    rm = (np.max(r_bias) + np.min(r_bias)) / 2.0
    return np.sqrt(mean_squared_error(lp + lm, y[~mask])), np.sqrt(
        mean_squared_error(rp + rm, y[mask])), lm, rm, model_id
    # return feval(model, X[left_mesh], y[~mask]), feval(model, X[right_mesh], y[mask])
...
class Node:
    ...
    def __init__(self, id=None, threshold=[],
                 ...
                 loss=None, model=None, classes=None, bias=0.0, model_id=None):
        self.id = id
...
class _LinearTree(BaseEstimator):
    ...
    def _split(self, X, y, bins,
        ...
        # select best results
        # left_node = (model_left, loss_left, wloss_left, n_left, largs_left['classes'])
        _id_best = np.argmin(_losses)
        # print(_losses[_id_best], loss)
        if not self._ext_models and _losses[_id_best] < loss:
            split_t = split_t[_id_best]
            split_col = split_col[_id_best]
            left_node = left_node[_id_best]
            right_node = right_node[_id_best]
            # left_node = (left_node[0], left_node[1], left_node[2], left_node[3], left_node[4], left_node[5], len(self._ext_models))
            # self._ext_models.append(left_node[0])
            # right_node = (right_node[0], right_node[1], right_node[2], right_node[3], right_node[4], right_node[5], len(self._ext_models))
            # self._ext_models.append(right_node[0])
        elif self._ext_models and _losses[_id_best] < loss:
            split_t = split_t[_id_best]
            split_col = split_col[_id_best]
            # testing existing models
            all_results = Parallel(n_jobs=n_jobs, verbose=0,
                                   **self._parallel_args())(
                delayed(_ext_model_fitting)(
                    X, y,
                    model_id, split_col, split_t, self
                )
                for model_id in range(len(self._ext_models)))
            loss_l, loss_r, p1, p2, mid = [], [], [], [], []
            for res in all_results:
                ...
                p2.append(res[3])
                mid.append(res[4])
            _least_loss_l = np.argmin(loss_l)
            _least_loss_r = np.argmin(loss_r)
            ln, rn = left_node[_id_best], right_node[_id_best]

            if (-left_node[_id_best][1] + loss_l[_least_loss_l]) < (-right_node[_id_best][1] + loss_r[_least_loss_r]):
                # print("inl")
                # print(_least_loss_r)
                if loss_l[_least_loss_l] + loss_r[_least_loss_r] < loss:
                    ln = (self._ext_models[_least_loss_l], loss_l[_least_loss_l], loss_l[_least_loss_l],
                          ln[3], ln[4], p1[_least_loss_l], mid[_least_loss_l])
                    rn = (self._ext_models[_least_loss_r], loss_r[_least_loss_r], loss_r[_least_loss_r],
                          rn[3], rn[4], p2[_least_loss_l], mid[_least_loss_r])
                elif loss_l[_least_loss_l] + right_node[_id_best][1] < loss:
                    ln = (self._ext_models[_least_loss_l], loss_l[_least_loss_l], loss_l[_least_loss_l],
                          ln[3], ln[4], p1[_least_loss_l], mid[_least_loss_l])
                    # a, b, c, d, e, f, g = rn
                    # rn = (a, b, c, d, e, f, len(self._ext_models))
                    # self._ext_models.append(rn[0])
                # else:
                #    a, b, c, d, e, f, g = ln
                #    ln = (a, b, c, d, e, f, len(self._ext_models))
                #    self._ext_models.append(a)
                #    a, b, c, d, e, f, g = rn
                #    rn = (a, b, c, d, e, f, len(self._ext_models))
                #    #self._ext_models.append(a)
            else:
                if loss_l[_least_loss_l] + loss_r[_least_loss_r] < loss:
                    ...
                    rn = (self._ext_models[_least_loss_r], loss_r[_least_loss_r], loss_r[_least_loss_r],
                          rn[3], rn[4], p2[_least_loss_l], mid[_least_loss_r])
                elif loss_r[_least_loss_r] + left_node[_id_best][1] < loss:
                    rn = (self._ext_models[_least_loss_r], loss_r[_least_loss_r], loss_r[_least_loss_r],
                          rn[3], rn[4], p2[_least_loss_l], mid[_least_loss_r])
                    # a, b, c, d, e, f, g = ln
                    # ln = (a, b, c, d, e, f, len(self._ext_models))
            ...
            split_t = None
            split_col = None
            left_node = (None, None, None, None, None, 0., -1)
            right_node = (None, None, None, None, None, 0., -1)
    ...
    def _grow(self, X, y, weights=None):
        ...
        while len(queue) > 0:
            ...
            if split_col is None or len(queue[-1]) >= self.max_depth:
                # print(queue[-1], self._nodes[queue[-1]].model_id)
                self._leaves[queue[-1]] = self._nodes[queue[-1]]
                self._leaves[queue[-1]].modify(len(self._ext_models))
                self._ext_models.append(self._nodes[queue[-1]].model)
                del self._nodes[queue[-1]]
                queue.pop()

            else:

                model_left, loss_left, wloss_left, n_left, class_left, bias, mid_l = \
                    left_node
                model_right, loss_right, wloss_right, n_right, class_right, bias, mid_r = \
                    right_node
                ...
                # print(self._nodes[queue[-1]].model_id)
