...
from DQDiscovery.LearnCRR.lineartree2._classes import _predict_branch
from DQDiscovery.LearnCRR.lineartree2._classes import _LinearTree, _LinearBoosting, _LinearForest
...
class LinearTreeRegressor(_LinearTree, RegressorMixin):
    ...
    def predict(self, X):
        ...
        for L in self._leaves.values():
            ...
            tot_mask += mask
            pred[mask] = L.model.predict(X[np.ix_(mask, self._linear_features)]) + L.bias

        return pred, tot_mask > 0
    
    def alg2_compation(self, X):
        check_is_fitted(self, attributes='_nodes')
        ...
        corresp = {i: [] for i in range(len(self._ext_models))}
        # for k in self._leaves:
        #    print(self._leaves[k].model_id)
        for L in self._leaves.values():
            ...
            corresp[L.model_id].append((mask, L.bias))
