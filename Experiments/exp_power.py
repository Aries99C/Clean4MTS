# -*- coding: utf-8 -*-
# @Author  : Genglong Li
# @Time    : 2023/7/12 20:41
# @Comment : Compare Clean4MTS with other benchmark times series data cleaning methods on the Power dataset
...
def performance_evaluation_clean4mts(mode='Clean4MTS'):
    path = r'..\Datasets\IDF_Power_Clean.csv'
    o = open(path)
    df = pd.read_csv(o)
    o.close()
    target = ['U3_DPU56A105']
    src = ['U3_HNC10AA101XQ01', 'U3_HNC10AN001XQ01', 'U3_HNC10CF101', 'U3_HNC10CG101XQ01']
    d_clean = np.array(df[src + target])

    path = r'..\Datasets\IDF_Power_Dirty.csv'
    o = open(path)
    df = pd.read_csv(o)
    o.close()
    ...
    with open('Save/rr_Power.pickle', 'rb') as f:
        rr = pickle.load(f)
        # 测试读取后的Model
    with open('Save/ac_Power.pickle', 'rb') as f:
        ac = pickle.load(f)
        # 测试读取后的Model
    with open('Save/sc_Power.pickle', 'rb') as f:
        sc = pickle.load(f)
    st = time.time()
    if mode == 'Clean4MTS+':
        pf = Clean4MTSPlus(db1, rr, sc, ac, 'Timestamp', target=target, src=src, delta_w=6, method='studGA', trappedValue=5e-4,
                       w1=7.5, w2=2.5, beta=1, print_repair_message=False)
...
def performance_evaluation_benchmark(mode='Vari'):
    ...
    db1 = DataBase(df)
    with open('.\Save\\vc_Power.pickle', 'rb') as f:
    ...
    if mode == 'Vari':
        ...
        path = r'..\Datasets\IDF_Power_Label.csv'
        o = open(path)
